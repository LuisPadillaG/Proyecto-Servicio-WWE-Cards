                ...
                fetch("http://localhost:3000/en_ascenso").then(recurso => recurso.json()).then(rango => {
                    ...
                    for (i = 0; i < 1; i++) { //realmente ni necesita el for, lo dejé por pura flojera
                        ...
                        fetch("http://localhost:3000/" + idCartaAleatoria).then(recurso => recurso.json()).then(tarjeta => {
                            ...
                            FiltroCarta.style.backgroundColor = "#b67d40d0"; //Este color es de rango bronce
                            NumeroPoderCarta.innerHTML = tarjeta.poder
                            IconoDivisionColor.src = "/recursos/icons/icon-emergente.png" //Esta imagen es de rango bronce
